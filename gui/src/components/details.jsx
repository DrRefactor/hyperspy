...
export default class Details extends Component {
  ...
  render() {
    const { node, summary = "" } = this.props
    let details = <DetailsTree node={node} summary={(summary ? summary + " " : "") +  "details"} />
    if (Array.isArray(node))
      details = node.map((x, i) => <DetailsTree node={x} key={i} summary="" />)
    return (
...
class DetailsTree extends Component {
  ...
  render() {
    ...
    const arrayElems = this.renderArrays(arrays)
    const objectElems = this.renderObjects(objects)
    const stringElems = this.renderStrings(strings)
  
    return (
      <details className='details-tree' open={!summary}>
        <summary>
          {summary}
        </summary>
  ...
  renderObjects(objects = []) {
    return objects.map(o => <DetailsTree key={o.summary} summary={o.summary} node={o.node} />)
  }
  renderArrays(arrays = []) {
    return arrays.map(a => {
      return (
        <div className='details-tree-list'>
          ...
          <ul>
            {a.nodes.map(n => <li>{<DetailsTree node={n} />}</li>)}
          </ul>
        </div>
      )
    })
